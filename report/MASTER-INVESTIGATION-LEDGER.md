...
- **SOURCE B** — Minimax/Mavis workspace `mvs_c19aa4…` (**does not exist** on this machine; verified via targeted search)
...
| GIT-003 | 2026-08-14 | C (R1) | Can Bug B be reproduced natively? | WorkBuddy-native PROBE_B (F1-shape) | **reproduced** `WORKTREE_ONLY_LOSS`, 59 unrelated tracked files, HEAD/index intact, HEAD_TREE==INDEX_TREE, fsck healthy | REPRODUCED | CONFIRMED | `work/native-runs/2026-08-14T21-43-52Z-8d51e361d168/PHASE1_FINAL_REPORT.md` | BUG-B | — | SYNTHETIC_NATIVE_REPRODUCTION=CONFIRMED |
| GIT-004 | 2026-08-14 | C (R1-repair) | Where in build does loss occur? | R1 repair rerun (CASE A) | loss localized to build's final `git checkout master`; WORKTREE_READY=NO before cycles | HIGH_CONFIDENCE_INFERENCE (localization) | HIGH | `work/native-runs/2026-08-14T15-32-33Z-jf10pgjgdoiu/REPAIR_FINAL_REPORT.md` | BUG-B | — | GIT_COMPONENT_CAUSE=UNRESOLVED |
| GIT-005 | 2026-08-15 | C (R2) | Reproduce again with 4 checkpoints? | R2 four-checkpoint harness | A/B/C/D all CLEAN, loss NOT reproduced in one controlled run | NEGATIVE CONTROL (this run) | CONFIRMED | `work/native-runs/2026-08-15T06-51-51Z-c7de59f17452/REPAIR_R2_FINAL_REPORT.md` | BUG-B | — | INTERMITTENT across runs |
| GIT-006 | 2026-08-15 | C (R2) | Which component causes Bug B? | synthesis of R1+R2 | no direct component-level evidence in either run | UNKNOWN (component) | UNRESOLVED | `REPAIR_R2_FINAL_REPORT.md` causal language | BUG-B | — | not attributable to a specific file |
| GIT-007 | 2026-08-13→15 | A/C | Is the tsbx kernel filter the cause? | inference from policy + negative controls | only remaining mechanism consistent with all facts; not directly observed denying an op | HIGH_CONFIDENCE_HYPOTHESIS | HIGH | `report/ROOT_CAUSE_CLOSURE_REPORT.md §B`, `REPAIR_R2_FINAL_REPORT.md` | BUG-B | — | do NOT upgrade to CONFIRMED |
| GIT-008 | 2026-08-15 | C (R2) | Is the loss WorkBuddy-associated? | native ancestry + R1 repro + R2 clean run | loss only under WorkBuddy-native chain; never from non-WB shell | PROVEN (association) | VERY_HIGH | `REPAIR_R2_FINAL_REPORT.md` `WORKBUDDY_RUNTIME_ASSOCIATION=VERY_HIGH` | BUG-B, EXECUTIVE-SUMMARY | — | |
...
| WB-R1-001 | 2026-08-14 | C (R1) | Native baseline harness valid? | GATE 0 + context + probes | GATE0=PASS; Probe A build interference (separate signal); Probe B repro | PROVEN (harness) | CONFIRMED | `PHASE1_FINAL_REPORT.md` | BUG-B | — | |
| WB-R2-001 | 2026-08-15 | C (R2) | Localize CLEAN→NON_CLEAN interval? | four-checkpoint build harness | A/B/C/D instrumented; all CLEAN in this run; harness proven correct | PROVEN (harness) | CONFIRMED | `REPAIR_R2_FINAL_REPORT.md`, `build-git-probe-f1-shape.ps1` | BUG-B | — | reviewer PASS |
...
| GIT-009 | 2026-08-14 | C (R1) | Is loss persistent across checkout-back? | Probe B step-1b | after switching back to master, 59 files STILL missing; not self-healing | PROVEN | CONFIRMED | `PHASE1_FINAL_REPORT.md` | BUG-B | — | distinguishes from transient scm state |
...
- HIGH_CONFIDENCE_INFERENCE: GIT-004, GIT-007
- HYPOTHESIS: GIT-007
- UNKNOWN: GIT-006
